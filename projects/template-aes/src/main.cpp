...
int main(void) {
  ...
  // Encrypt a message
  status = aes.encrypt(input, buffer, (uint16_t*)sizeof(input));
    if (status)
    {
        led_green.on();
    ...
    // check encrypted buffer
    char bufferString[16];
    sprintf(bufferString, "%02X", buffer[0]);
    for (int i = 0; i < 16; i++)
        sprintf(bufferString + strlen(bufferString), "-%02X", buffer[i]);
    sprintf((char*) uartBuffer, "Messaggio criptato: %s", bufferString);

    // Decrypt a message
    status = aes.decrypt(output, buffer, (uint16_t*)sizeof(output));
    if (status)
    {
        led_orange.on();
    ...
    for (int i = 0; i < 16; i++)
        sprintf(bufferString + strlen(bufferString), "-%02X", buffer[i]);
    sprintf((char*) uartBuffer, "Messaggio decriptato: %s", bufferString);
